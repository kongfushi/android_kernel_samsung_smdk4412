...
static int qc_prepare_download(struct dpram_link_device *dpld)
{
	...
	while (1) {
		if (dpld->qc_udl_check.copy_start) {
			dpld->qc_udl_check.copy_start = 0;
			break;
		}
			...
			enum qc_dload_tag tag)
{
	...
	dpld->qc_udl_check.total_size = param.size;
	dpld->qc_udl_check.rest_size = param.size;
	dpld->qc_udl_check.send_size = 0;
	dpld->qc_udl_check.copy_complete = 0;

	dpld->qc_udl_param.addr = img;
	dpld->qc_udl_param.size = dpld->dpram->max_boot_frame_size;
	if (tag == QC_DLOAD_TAG_NV)
		dpld->qc_udl_param.count = 1;
	else
		dpld->qc_udl_param.count = param.count;
	dpld->qc_udl_param.tag = tag;

	if (dpld->qc_udl_check.rest_size < dpld->dpram->max_boot_frame_size)
		dpld->qc_udl_param.size = dpld->qc_udl_check.rest_size;

	/* Download image (binary or NV) */
	qc_do_download(dpld, &dpld->qc_udl_param);
	...
	*/
	...
	while (1) {
		if (dpld->qc_udl_check.copy_complete) {
			dpld->qc_udl_check.copy_complete = 0;
			retval = 0;
			break;
		}
		...
		count++;
		if (count > cnt_limit) {
			dpld->qc_udl_check.total_size = 0;
			dpld->qc_udl_check.rest_size = 0;
			mif_err("ERR! count %d\n", count);
			retval = -1;
			break;
		}
...
static void qc_dload_task(unsigned long data)
{
	...
	dpld->qc_udl_check.send_size += dpld->qc_udl_param.size;
	dpld->qc_udl_check.rest_size -= dpld->qc_udl_param.size;

	dpld->qc_udl_param.addr += dpld->qc_udl_param.size;

	if (dpld->qc_udl_check.send_size >= dpld->qc_udl_check.total_size) {
		dpld->qc_udl_check.copy_complete = 1;
		dpld->qc_udl_param.tag = 0;
		return;
	}
...
static void qc_dload_cmd_handler(struct dpram_link_device *dpld, u16 cmd)
{
	switch (cmd) {
	case 0x1234:
		dpld->qc_udl_check.copy_start = 1;
		break;

	case 0xDBAB:
		if (dpld->qc_udl_check.total_size)
			tasklet_schedule(&dpld->dl_tsk);
		break;

	case 0xABCD:
		dpld->qc_udl_check.boot_complete = 1;
		break;
...
static int qc_boot_start(struct dpram_link_device *dpld)
{
	...
	while (1) {
		if (dpld->qc_udl_check.boot_complete) {
			dpld->qc_udl_check.boot_complete = 0;
			break;
		}
