...
#include <linux/platform_data/modem_chn.h>
#include "modem_prj.h"
...
extern unsigned char is_forced_cp_crash;
...
static irqreturn_t phone_active_irq_handler(int irq, void *arg)
{
	...
	if (phone_reset && phone_active) {
		...
		if (mc->phone_state == STATE_ONLINE) {
			phone_state = STATE_CRASH_RESET; // silent reset, STATE_CRASH_EXIT;
			if (mc->iod && mc->iod->modem_state_changed)
	...
	else if (!phone_reset && !phone_active && is_forced_cp_crash == 1) {
		if (mc->phone_state == STATE_ONLINE) {
