...
static int __devinit sec_battery_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&battery->vbus_wake_lock, WAKE_LOCK_SUSPEND,
		       "sec-battery-vbus");
	wake_lock_init(&battery->bootcomplete_wake_lock, WAKE_LOCK_SUSPEND,
		       "sec-bootcomplete_wake");
	...
	wake_lock_timeout(&battery->bootcomplete_wake_lock, HZ * 60);
	dev_dbg(battery->dev,
...
err_wake_lock:
	...
	wake_lock_destroy(&battery->bootcomplete_wake_lock);
